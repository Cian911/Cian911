...
dkube() {
   kubectl get pods -n $1 | grep "$2" | awk '{print $1}' | xargs kubectl delete pods -n $1
}

# Switch k8s namespace context
sns() {
  kubectl config set-context $(kubectl config current-context) --namespace=$1
}

gpgd() {
   echo -n "$1" | base64 -d | gpg --decrypt
}
